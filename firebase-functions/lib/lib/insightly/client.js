...
exports.createInsightlyLead = createInsightlyLead;
...
function validateResponse(data) {
    ...
    if (typeof response.LEAD_ID !== 'number') {
        throw new Error(`[Insightly] Invalid response: missing or invalid LEAD_ID. Received: ${JSON.stringify(response)}`);
    }
    return response;
}
async function makeRequestWithRetry(url, options, retryConfig = config_1.insightlyDefaults.retryConfig) {
    let lastError = null;
        ...
        try {
            ...
            if (response.status === 429) {
                ...
                if (attempt < retryConfig.maxRetries) {
                    console.warn(`[Insightly API] Rate limit exceeded (429). Retrying after ${retryDelay}ms (attempt ${attempt + 1}/${retryConfig.maxRetries})`);
            ...
            if (attempt < retryConfig.maxRetries) {
                console.warn(`[Insightly API] Request failed, retrying after ${delay}ms (attempt ${attempt + 1}/${retryConfig.maxRetries})`, { error: lastError.message });
                await sleep(delay);
                delay = Math.min(delay * retryConfig.backoffMultiplier, retryConfig.maxDelayMs);
            }
            else {
...
async function createInsightlyLead(payload) {
    const { apiUrl, apiKey } = (0, config_1.getInsightlyApiConfig)();
    ...
    if (!response.ok) {
        ...
        throw new Error(`[Insightly] Failed to create lead (${response.status} ${response.statusText}) ${errorBody}`);
    }
